...
`
export const DELETE_RETARD = gql`
  mutation DeleteRetard($id: ID!) {
    deleteRetard(id: $id)
  }
`
export const DELETE_TENUE_TRAVAIL = gql`
  mutation DeleteTenueTravail($id: ID!) {
    deleteTenueTravail(id: $id)
  }
`
// New: Get all user work schedules for all employees for a date range (for main planning table)
export const GET_ALL_USER_WORK_SCHEDULES = gql`
...
`
// New: Create User Work Schedule (per-employee table)
export const CREATE_USER_WORK_SCHEDULE = gql`
