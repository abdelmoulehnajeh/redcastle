...
import { GET_DASHBOARD_DATA, CREATE_EMPLOYEE, DELETE_EMPLOYEE, UPDATE_EMPLOYEE } from "@/lib/graphql-queries"
...
const translations: Record<Lang, Dict> = {
  fr: {
    ...
    statsHoursThisMonth: "Jours ce mois",
  ...
  ar: {
    ...
    statsHoursThisMonth: "الايام هذا الشهر",
...
export default function DashboardPage() {
  ...
  // Use combined dashboard data query - single API call for all dashboard data
  const { data, loading, refetch } = useQuery(GET_DASHBOARD_DATA, {
  ...
  if (loading) {
    return (
      <div className="min-h-screen bg-gradient-to-br from-slate-900 via-purple-900 to-slate-900 relative overflow-hidden">
        ...
        <div className="relative z-10 p-3 sm:p-6 space-y-4 sm:space-y-8">
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-3 sm:gap-6">
            {[...Array(4)].map((_, i) => (
              <div
                key={i}
                className="group relative bg-gradient-to-br from-white/10 to-white/5 backdrop-blur-lg border border-white/20 rounded-2xl p-4 sm:p-6 animate-pulse transform hover:scale-105 transition-all duration-300"
                style={{
    ...
    >
      ...
      <div className="relative z-10 p-3 sm:p-6 space-y-4 sm:space-y-8">
        ...
        <div className="group relative transform-gpu perspective-1000">
          <div
            className="relative bg-gradient-to-r from-red-600/90 via-red-500/90 to-orange-600/90 backdrop-blur-xl border border-red-400/30 rounded-2xl sm:rounded-3xl p-4 sm:p-8 text-white transition-all duration-500"
            style={{
          ...
          >
            <div className="absolute -top-10 sm:-top-20 -right-10 sm:-right-20 w-20 sm:w-40 h-20 sm:h-40 bg-gradient-to-br from-yellow-400/20 to-red-400/20 rounded-full blur-3xl animate-pulse"></div>
            <div className="relative z-10">
              <h1
                dir="auto"
                className="text-2xl sm:text-4xl lg:text-5xl font-bold mb-2 sm:mb-4 bg-gradient-to-r from-white via-yellow-100 to-white bg-clip-text text-transparent leading-tight"
              >
                {t.welcome(user?.username)}
              </h1>
              <p dir="auto" className="text-red-100 text-sm sm:text-lg lg:text-xl mb-4 sm:mb-6 leading-relaxed">
                {user?.role === "admin" ? t.introAdmin : user?.role === "manager" ? t.introManager : t.introEmployee}
              </p>
              <Badge
                variant="secondary"
                className="mt-2 sm:mt-3 bg-white/20 text-white border-white/30 px-3 sm:px-6 py-1 sm:py-2 text-sm sm:text-lg font-semibold backdrop-blur-sm transition-all duration-300"
              >
            ...
            <>
              <Link href="/dashboard/admin/employees">
                <StatsCard3D
                  ...
                  subtitle={t.statsOutOfEmployees(
                    stats.activeEmployees || employees.length,
        ...
        {user?.role === "admin" && (
          <Card3D>
            <CardHeader className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4">
              <div>
                <CardTitle className="flex items-center gap-2 sm:gap-3 text-xl sm:text-2xl font-bold bg-gradient-to-r from-red-400 to-orange-400 bg-clip-text text-transparent">
                  <MapPin className="h-5 w-5 sm:h-7 sm:w-7 text-red-400" />
            ...
            <CardContent>
              {locations.length > 0 ? (
                ...
                <div className="text-center py-8 sm:py-12 text-gray-400">
                  <MapPin className="h-12 w-12 sm:h-16 sm:w-16 mx-auto mb-4 sm:mb-6 opacity-50" />
          ...
          <Card3D>
            ...
            <CardContent className="px-2 sm:px-6">
              {filteredEmployees.length > 0 ? (
                      ...
                      >
                        <div className="flex items-start justify-between mb-3">
                          <div className="flex-1">
                            <h3 className="font-semibold text-white text-base sm:text-lg" dir="auto">
                    ...
                    <Table>
                      <TableHeader>
                        <TableRow className="border-slate-600 hover:bg-slate-700/50">
                          <TableHead className="text-gray-300 font-semibold" dir="auto">
                            {t.employeeCol}
                          </TableHead>
                          <TableHead className="text-gray-300 font-semibold" dir="auto">
                            {t.contactCol}
                      ...
                      <TableBody>
                        {filteredEmployees.map((employee: any) => (
                          <TableRow
                            key={employee.id}
                            className="border-slate-600/50 hover:bg-slate-700/30 transition-all duration-300"
                          >
                            <TableCell>
                              <div>
                            ...
                            <TableCell className="text-gray-300" dir="auto">
                              {employee.job_title || t.notDefined}
                            </TableCell>
                            <TableCell>
                              {employee.location ? (
                ...
                <div className="text-center py-8 sm:py-12 text-gray-400">
                  <Users className="h-12 w-12 sm:h-16 sm:w-16 mx-auto mb-4 sm:mb-6 opacity-50" />
                  <p className="text-lg sm:text-xl" dir="auto">
                    {t.noEmployees}
                  </p>
                </div>
        ...
        <Dialog open={isEditEmployeeOpen} onOpenChange={setIsEditEmployeeOpen}>
          <DialogContent className="w-[95vw] max-w-md mx-auto bg-slate-800/95 backdrop-blur-xl border border-slate-600/50 rounded-2xl text-white">
            <DialogHeader>
              <DialogTitle
                className="text-lg sm:text-2xl font-bold bg-gradient-to-r from-emerald-400 to-cyan-400 bg-clip-text text-transparent"
                dir="auto"
              >
                {t.editEmployeeTitle}
              </DialogTitle>
            ...
            {selectedEmployee && (
              <div className="space-y-4 max-h-[60vh] overflow-y-auto">
                <div className="grid grid-cols-2 gap-4">
                  <div>
                    <Label htmlFor="edit-salaire" className="text-gray-300 text-sm" dir="auto">
                      {t.salary}
                    </Label>
                    <Input
                      id="edit-salaire"
                  ...
                  <div>
                    <Label htmlFor="edit-prime" className="text-gray-300 text-sm" dir="auto">
                      {t.bonus}
                    </Label>
                    <Input
                      id="edit-prime"
                ...
                <div className="grid grid-cols-3 gap-4">
                  <div>
                    <Label htmlFor="edit-infractions" className="text-gray-300 text-sm" dir="auto">
                      {t.fines}
                    </Label>
                    <Input
                      id="edit-infractions"
                  ...
                  <div>
                    <Label htmlFor="edit-absence" className="text-gray-300 text-sm" dir="auto">
                      {t.absences}
                    </Label>
                    <Input
                      id="edit-absence"
                  ...
                  <div>
                    <Label htmlFor="edit-retard" className="text-gray-300 text-sm" dir="auto">
                      {t.delays}
                    </Label>
                    <Input
                      id="edit-retard"
                ...
                <div className="grid grid-cols-2 gap-4">
                  <div>
                    <Label htmlFor="edit-avance" className="text-gray-300 text-sm" dir="auto">
                      {t.advances}
                    </Label>
                    <Input
                      id="edit-avance"
                  ...
                  <div>
                    <Label htmlFor="edit-tenu" className="text-gray-300 text-sm" dir="auto">
                      {t.uniforms}
                    </Label>
                    <Input
                      id="edit-tenu"
                ...
                <div>
                  <Label htmlFor="edit-status" className="text-gray-300 text-sm" dir="auto">
                    {t.statusCol}
                  </Label>
                  <Select
        ...
        <Card3D>
          <CardHeader>
            <CardTitle className="text-lg sm:text-2xl font-bold bg-gradient-to-r from-purple-400 to-pink-400 bg-clip-text text-transparent flex items-center gap-2 sm:gap-3">
          ...
          <CardContent className="px-3 sm:px-8">
            {/* Mobile */}
...
function StatsCard3D({ title, value, subtitle, icon, gradient, glowColor }: any) {
  return (
    <div className="group relative transform-gpu perspective-1000">
      <div
        className={`relative bg-gradient-to-br ${gradient} backdrop-blur-xl border border-white/20 rounded-2xl p-4 sm:p-6 text-white transform hover:rotateX-12 hover:rotateY-6 hover:scale-105 transition-all duration-500 cursor-pointer`}
        style={{
          transform: "perspective(1000px) rotateX(10deg) rotateY(-5deg)",
      ...
      >
        ...
        <div className="relative z-10">
          <div className="flex items-center justify-between mb-3 sm:mb-4">
            <div className="text-white/80 font-medium text-xs sm:text-sm tracking-wide uppercase" dir="auto">
              {title}
            </div>
            <div className="text-white/60 group-hover:text-white/80 transition-colors duration-300">{icon}</div>
          </div>
          <div className="text-2xl sm:text-3xl font-bold mb-1 sm:mb-2 bg-gradient-to-r from-white to-white/80 bg-clip-text text-transparent">
            {value}
          </div>
          <div className="text-white/70 text-xs sm:text-sm" dir="auto">
            {subtitle}
          </div>
        </div>
...
function Card3D({ children, className = "" }: any) {
  return (
    <div className="group relative transform-gpu perspective-1000">
      <div
        className={`relative bg-slate-800/40 backdrop-blur-xl border border-slate-600/50 rounded-2xl sm:rounded-3xl text-white transform hover:rotateX-2 hover:rotateY-1 transition-all duration-500 ${className}`}
        style={{
      ...
      >
        <div className="absolute inset-0 rounded-2xl sm:rounded-3xl bg-gradient-to-r from-purple-400/20 via-cyan-400/20 to-purple-400/20 opacity-0 group-hover:opacity-100 transition-opacity duration-500 blur-sm"></div>
...
function LocationCard3D({ location, isLoading = false, t }: any) {
  if (isLoading) {
    return (
      <div className="relative bg-slate-700/30 backdrop-blur-lg border border-slate-600/50 rounded-2xl p-4 sm:p-6 animate-pulse">
  ...
  return (
    <div className="group relative transform-gpu perspective-1000">
      <div
        className="relative bg-gradient-to-br from-slate-700/50 to-slate-800/50 backdrop-blur-xl border border-slate-600/50 rounded-2xl p-4 sm:p-6 text-white transform hover:rotateX-6 hover:rotateY-3 hover:scale-105 transition-all duration-500"
        style={{
      ...
      >
        ...
        <div className="relative z-10">
          <div className="mb-4">
            ...
            <p className="text-gray-400 flex items-center gap-2 mt-2 text-sm" dir="auto">
          ...
          <div className="space-y-3">
            <div className="flex justify-between items-center">
              <span className="text-gray-300 text-sm" dir="auto">
                {t.employeesLabel}
              </span>
            ...
            {location.manager && (
              <div className="flex justify-between items-center">
                ...
                <span className="font-medium text-white text-sm" dir="auto">
            ...
            <div className="mt-4 sm:mt-6">
              <Button
                asChild
                className="w-full bg-gradient-to-r from-red-500 to-orange-500 hover:from-red-600 hover:to-orange-600 text-white font-semibold rounded-xl transition-all duration-300 text-sm"
...
function QuickActionMobile({ href, icon, title, subtitle, gradient }: any) {
  return (
    <Link href={href} className="block">
      <div
        className={`relative bg-gradient-to-r ${gradient} backdrop-blur-sm border border-white/20 rounded-xl p-4 text-white hover:scale-[1.02] active:scale-95 transition-all duration-200 shadow-lg`}
      >
        ...
        <div className="relative z-10 flex items-center gap-4">
          ...
          <div className="flex-1 min-w-0">
            <h3 className="font-semibold text-white text-base leading-tight" dir="auto">
              {title}
            </h3>
            <p className="text-white/80 text-sm mt-1 leading-tight" dir="auto">
              {subtitle}
            </p>
          </div>
      ...
      <Link href={href}>
        ...
        >
          ...
          <div className="relative z-10 flex flex-col items-center space-y-2 sm:space-y-3">
            ...
            <span
              className="text-xs sm:text-sm font-semibold text-center text-white/90 group-hover:text-white transition-colors duration-300 leading-tight"
              dir="auto"
            >
              {title}
            </span>
