...
import {
  ...
  MANAGER_REQUEST_EMPLOYEE_UPDATE,
  MANAGER_REQUEST_CREATE_INFRACTION,
  MANAGER_REQUEST_CREATE_ABSENCE,
  MANAGER_REQUEST_CREATE_RETARD,
  MANAGER_REQUEST_CREATE_TENUE,
  MANAGER_REQUEST_DELETE_INFRACTION,
  MANAGER_REQUEST_DELETE_ABSENCE,
  MANAGER_REQUEST_DELETE_RETARD,
  MANAGER_REQUEST_DELETE_TENUE,
  MANAGER_REQUEST_UPDATE_ROLE,
...
export default function EmployeeDetailsPage() {
  ...
  const [managerRequestEmployeeUpdate] = useMutation(MANAGER_REQUEST_EMPLOYEE_UPDATE)
  const [managerRequestCreateInfraction] = useMutation(MANAGER_REQUEST_CREATE_INFRACTION)
  const [managerRequestCreateAbsence] = useMutation(MANAGER_REQUEST_CREATE_ABSENCE)
  const [managerRequestCreateRetard] = useMutation(MANAGER_REQUEST_CREATE_RETARD)
  const [managerRequestCreateTenue] = useMutation(MANAGER_REQUEST_CREATE_TENUE)
  const [managerRequestDeleteInfraction] = useMutation(MANAGER_REQUEST_DELETE_INFRACTION)
  const [managerRequestDeleteAbsence] = useMutation(MANAGER_REQUEST_DELETE_ABSENCE)
  const [managerRequestDeleteRetard] = useMutation(MANAGER_REQUEST_DELETE_RETARD)
  const [managerRequestDeleteTenue] = useMutation(MANAGER_REQUEST_DELETE_TENUE)
  const [managerRequestUpdateRole] = useMutation(MANAGER_REQUEST_UPDATE_ROLE)
  ...
  const handleAddDisciplinaryItem = async (type: string) => {
    ...
    try {
      switch (type) {
        case "infraction":
          await managerRequestCreateInfraction({ variables })
          break
        case "absence":
          await managerRequestCreateAbsence({ variables })
          break
        case "retard":
          await managerRequestCreateRetard({ variables })
          break
        case "tenue":
          await managerRequestCreateTenue({ variables })
          break
      }

      toast.success("Demande envoyée", {
        description: "Votre demande a été envoyée à l'administrateur pour approbation",
      })

      setDisciplinaryForms((prev) => ({
        ...prev,
        [type]: { name: "", price: "", description: "", date: undefined },
      }))
    } catch (error) {
  ...
  const handleSave = async () => {
    try {
      ...
      if (editData.username && editData.username !== employee?.user?.username) {
        changes.userInfo.username = editData.username.trim()
      }
      if (editData.hire_date && editData.hire_date !== employee?.created_at?.split("T")[0]) {
        changes.userInfo.hire_date = editData.hire_date
      }
      if (editData.password && editData.password.trim()) {
        changes.userInfo.password = editData.password.trim()
      }
      ...
      const oldValues = {
        personal: {
          ...
          email: employee?.email || "",
          telephone: employee?.telephone || "",
          location_id: employee?.location?.id || null,
          job_title: employee?.job_title || "",
        },
        financial: {
          salaire: employee?.salaire || 0,
        ...
        userInfo: {
          username: employee?.user?.username || "",
          hire_date: employee?.created_at?.split("T")[0] || "",
        },
      }

      // Send approval request to admin
      await managerRequestEmployeeUpdate({
        variables: {
          employee_id: employeeId,
          manager_id: user?.id,
      ...
      // Handle role change separately if needed
      if ((editData.role ?? role) && employee?.user?.role !== (editData.role ?? role)) {
        await managerRequestUpdateRole({
          variables: {
            user_id: employee.user.id,
            employee_id: employeeId,
            manager_id: user?.id,
            old_role: employee?.user?.role || "employee",
            new_role: editData.role ?? role,
            month: new Date().toISOString().slice(0, 7),
          },
        })
      }
  ...
  const handleDeleteDisciplinaryItem = async (type: string, id: string) => {
    try {
      const month = new Date().toISOString().slice(0, 7)

      switch (type) {
        case "infraction":
          await managerRequestDeleteInfraction({
            variables: {
              infraction_id: id,
              employee_id: employeeId,
              manager_id: user?.id,
          ...
          break
        case "absence":
          await managerRequestDeleteAbsence({
            variables: {
              absence_id: id,
              employee_id: employeeId,
              manager_id: user?.id,
          ...
          break
        case "retard":
          await managerRequestDeleteRetard({
            variables: {
              retard_id: id,
              employee_id: employeeId,
              manager_id: user?.id,
          ...
          break
        case "tenue":
          await managerRequestDeleteTenue({
            variables: {
              tenue_id: id,
              employee_id: employeeId,
              manager_id: user?.id,
      ...
      toast.error("Erreur lors de l'envoi de la demande")
      console.error(`Error requesting delete ${type}:`, error)
    }
  }
                ...
                {isEditing ? (
                  ...
                  <div className="flex items-center space-x-2">
                    ...
                    <span className="text-sm sm:text-base font-semibold text-foreground" dir="auto">
                      {employee.location?.name || getLocationName(employee.location?.id) || "Non assigné"}
                    </span>
                  </div>
                )}
              </div>
                          ...
                          <div className="flex justify-between items-start">
                            ...
                            <div className="text-right flex items-start space-x-2">
                              ...
                              <Button
                                ...
                                onClick={() => handleDeleteDisciplinaryItem("infraction", item.id)}
                              >
                                <X className="h-4 w-4" />
                              </Button>
                            </div>
                          ...
                          <div className="flex justify-between items-start">
                            ...
                            <div className="text-right flex items-start space-x-2">
                              ...
                              <Button
                                variant="ghost"
                                size="icon"
                                className="h-6 w-6 text-orange-400 hover:text-orange-600 hover:bg-orange-500/20"
                                onClick={() => handleDeleteDisciplinaryItem("absence", item.id)}
                              >
                                <X className="h-4 w-4" />
                              </Button>
                            </div>
                          ...
                          <div className="flex justify-between items-start">
                            ...
                            <div className="text-right flex items-start space-x-2">
                              ...
                              <Button
                                variant="ghost"
                                size="icon"
                                className="h-6 w-6 text-yellow-400 hover:text-yellow-600 hover:bg-yellow-500/20"
                                onClick={() => handleDeleteDisciplinaryItem("retard", item.id)}
                              >
                                <X className="h-4 w-4" />
                              </Button>
                            </div>
                          ...
                          <div className="flex justify-between items-start">
                            ...
                            <div className="text-right flex items-start space-x-2">
                              ...
                              <Button
                                variant="ghost"
                                size="icon"
                                className="h-6 w-6 text-blue-400 hover:text-blue-600 hover:bg-blue-500/20"
                                onClick={() => handleDeleteDisciplinaryItem("tenue", item.id)}
                              >
                                <X className="h-4 w-4" />
                              </Button>
                            </div>
